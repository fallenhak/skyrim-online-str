...
| `ClientReferencesMoveRequest` / `CharacterService` | Movement and action replay cache | map key is server entity ID | yes | no per-entry epoch | `OwnerView` checks owner pointer; no range or finite-value validation | B/E | Add per-entry epoch or otherwise bind updates to an ownership incarnation; bound/validate payloads | No |
