...
No Skyrim `.ess` file is loaded or written for character entry. From the title screen, the client opens the native console and runs `coc WhiterunTempleofKynareth`; it waits for that interior cell to finish loading, then applies the owner-validated snapshot and continues through the existing ready/assignment handshake. This title-screen COC path is documented by the SkyMP plugin example, which enters the game from the main menu with `coc riverwood` ([reference](https://github.com/skyrim-multiplayer/skymp/blob/main/skyrim-platform/tools/plugin-example/README.md)). The new character's initial cell is Skyrim.esm `WhiterunTempleofKynareth` (`000165A7`); the client uses the cell's vanilla COC marker as the spawn location. The ready request carries that marker position only for first placement, and the server persists it before assignment. On final assignment the server reloads the selected record owner-scoped and its V1 name, level, worldspace, cell, position, and current health/magicka/stamina override client assignment values. Authentication fields such as username, level, worldspace, cell, time, and Discord ID do not override the persisted record.
...
After authenticated connection, the client opens Character Select and requests slot capacity followed by a complete character list. The UI renders only the returned `CharacterSummary` values and sends a selection request only for an ID from that list. The default capacity is three slots, with one unlocked. A create request contains only the slot index and name; the server validates the UTF-8 name, owner-scoped uniqueness, slot lock, and occupancy before inserting a record. Successful creation immediately enters the same snapshot/ready/assignment sequence as selection.

For a new record, the client completes that server assignment first, then opens Skyrim's native RaceMenu without starting `MQ101`. When the menu closes, the client sends only the chosen race and sex; the server updates those V1 fields and clears `NeedsRaceMenu`. Returning characters do not open RaceMenu. Appearance data beyond race and sex is not persisted in this milestone.
