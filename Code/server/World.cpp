    ...
    const char* apRaceClassificationOverrides, const std::uint32_t aCreatureCorpseLifetimeSeconds)
{
    ...
    ctx().emplace<CalendarService>(*this, m_dispatcher, ctx().at<PersistenceService>().GetWorldClockRepository());
    ...
    ctx().emplace<LeveledActorPicker>(m_recordCollection.get(), ctx().at<EncounterZoneIndex>());
