...
TEST_CASE("Character session protocol messages round trip", "[encoding.character_session]")
{
    ...
    SECTION("server list and selection result")
    {
        ...
        REQUIRE(static_cast<unsigned>(kNotifyWorldItemTaken) == static_cast<unsigned>(kNotifyObjectHarvested) + 1);
        REQUIRE(static_cast<unsigned>(kNotifyFurnitureUseDenied) == static_cast<unsigned>(kNotifyWorldItemTaken) + 1);
        REQUIRE(static_cast<unsigned>(kNotifyContainerTransferResult) == static_cast<unsigned>(kNotifyFurnitureUseDenied) + 1);
        REQUIRE(static_cast<unsigned>(kNotifyCorpseContents) == static_cast<unsigned>(kNotifyContainerTransferResult) + 1);
        REQUIRE(static_cast<unsigned>(kNotifyActorInventory) == static_cast<unsigned>(kNotifyCorpseContents) + 1);
        REQUIRE(static_cast<unsigned>(kServerOpcodeMax) == static_cast<unsigned>(kNotifyActorInventory) + 1);
        ...
        REQUIRE(static_cast<unsigned>(kRequestActorInventory) == static_cast<unsigned>(kObjectStateReport) + 1);
        REQUIRE(static_cast<unsigned>(kClientOpcodeMax) == static_cast<unsigned>(kRequestActorInventory) + 1);
    }
}
